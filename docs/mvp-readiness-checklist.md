...
この資料は、`spec.md` の「19. 完成条件」を第10段階時点の実装、テスト、手動確認結果と照合するための記録である。
...
| 17 | 音名と指番号の表示を切り替えられる | 要注意 | 現状は音名と指番号を表示できるが、ON/OFF切替UIは未実装 | 仕様どおりMVP必須にするなら小規模追加実装が必要。教室試用で必須でないなら `spec.md` の完成条件を「表示できる」へ修正する判断が必要 |
...
| 23 | 自動テストが成功する | 達成済み / 自動テストで確認済み | 第10段階で型検査、単体テスト、E2E、ビルド、`git diff --check` を実行 | なし |
...
次は `spec.md` 上もMVP対象外または将来検討であり、第10段階では実装しない。
...
1. **完成条件17の表示切替UI**
   - 現状は音名と指番号を表示できるが、ON/OFF切替はない。
   - 厳密に完成条件を満たすには追加実装が必要。
   - 推奨案: 教室試用前に必須なら小規模追加実装を行う。必須でなければ仕様文言を「表示できる」へ修正する。
