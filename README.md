...
このプロジェクトは現在、第7段階「メトロノームとプリカウント」と再生速度変更時の同期補正まで実装済みである。PC確認で、赤い判定線があると音楽ゲーム的な完全同期を期待させることが分かったため、第7.5段階として再生ガイド表示とプリカウント中の助走表示を見直す状態にある。音源再生、演奏判定、設定保存は後続段階で実装する。
...
- 第5.5段階 横表示の表示調整: 完了
- 第6段階 共通タイムラインと再生: 完了
- 第7段階 メトロノームとプリカウント: 補正実装済み
- 第7.5段階 再生ガイドと助走表示: 実装開始待ち
- 音源再生、演奏判定、設定保存: 未実装
- 自動テスト環境: 構築済み
