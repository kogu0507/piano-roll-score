...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の端末別レイアウト補正実装開始待ちである。追加補正により表示調整フロートと音価の幅は実装されたが、PCとタブレットでは表示調整が使いにくく、スマートフォンでは練習メニューと表示調整フロートの閉じやすさ、位置、透明度に課題が残った。今後はスマートフォン、Androidタブレット、Windowsタブレット、PCで同じ機能を保ちつつ、画面サイズ別にレイアウトを分けて調整する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 端末別レイアウト補正実装開始待ち
