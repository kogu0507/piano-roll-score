...
このプロジェクトは現在、第8段階「端末内保存と設定保存」まで完了し、第9段階「レスポンシブ対応と操作性調整」の実装開始待ちである。次は、スマートフォン、タブレット、PCで既存機能を無理なく操作できるように画面全体を調整する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
