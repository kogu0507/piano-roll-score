...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の再補正実装開始待ちである。直近補正ではPC表示は概ね成立したが、スマートフォンでは操作欄がCanvasを圧迫し、実鍵盤を見ながら調整しにくいことが分かった。第10段階へ進む前に、PC、スマートフォン、タブレットで同じ運用ができるよう、通常練習モードと表示調整モードを分け、Canvasをピアノ鍵盤に近い画面下側へ大きく配置する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
