...
このプロジェクトは現在、第7.5段階「再生ガイドと助走表示」まで実装済みで、PC・スマートフォンでの手動確認待ちである。赤い判定線を弱い再生ガイドへ変更し、プリカウント中も表示用の助走位置で譜面を流すようにした。音源再生、演奏判定、設定保存は後続段階で実装する。
