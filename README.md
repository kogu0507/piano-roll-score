...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の端末別レイアウト補正実装が完了し、ユーザー確認待ちである。スマートフォン幅の練習メニューは80%程度の右ドロワーになり、背景タップで閉じられるようになった。表示調整フロートは画面基準の固定配置へ変更され、PC/タブレットでは右側サイド寄せ、スマートフォン横向きでは上部寄せの配置になった。第10段階へ進む前に、PC、スマートフォン、Androidタブレット、可能であればWindowsタブレットで確認する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 端末別レイアウト補正実装済み、ユーザー確認待ち
