...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の再補正実装が完了し、ユーザー確認待ちである。再補正後の練習画面は、通常練習モードと表示調整モードを分け、Canvasをピアノ鍵盤に近い画面下側へ大きく配置する方針になっている。第10段階へ進む前に、PC、スマートフォン、可能であればタブレットで、ピアノ上に端末または小型ディスプレイを置く前提の操作感を確認する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 再補正実装済み、ユーザー確認待ち
