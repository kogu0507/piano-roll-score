...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の追加補正実装が完了し、ユーザー確認待ちである。表示調整パネルはCanvas上の半透明フローティング表示へ変更され、縦表示・横表示に「音価の幅」50〜200%調整が追加された。第10段階へ進む前に、PC、Android Chrome、可能であればタブレットで、Canvasを見ながら表示調整できるか、速いテンポでも音符の動きを追いやすいかを確認する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 追加補正実装済み、ユーザー確認待ち
