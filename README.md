...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の再々補正実装開始待ちである。直近の再補正でPC表示は合格したが、スマートフォンでは操作ボタンと再生速度欄が大きすぎてCanvasを圧迫し、練習メニューの多くが画面外になった。第10段階へ進む前に、スマートフォン実機で使える最小アイコン操作列、再生速度プルダウン、低背シークバー、画面内に収まる練習メニューへ補正する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 再々補正実装開始待ち
