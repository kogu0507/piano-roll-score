...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」まで実装済みで、PC・スマートフォン・可能であればタブレットでの手動確認待ちである。スマートフォンやタブレットを鍵盤付近へ置いて使う前提で、譜面Canvasと練習中によく使う操作を画面の主役にした。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
