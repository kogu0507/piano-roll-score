...
このプロジェクトは現在、第8段階「端末内保存と設定保存」まで実装済みで、PC・スマートフォンでの手動確認待ちである。利用者が明示的に保存した楽曲を端末内へ保存し、表示・再生設定を次回起動時に復元できるようにした。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
