...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の補正実装が完了し、ユーザー確認待ちである。補正後の練習画面は「上部: 曲名・切替・メニュー、中央: Canvas最大表示、下部: 再生・表示調整バー」という構造になっている。第10段階へ進む前に、PC、スマートフォン、可能であればタブレットで、スマートフォンやタブレットを鍵盤付近へ置いて使う前提の操作感を確認する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 補正実装済み、ユーザー確認待ち
