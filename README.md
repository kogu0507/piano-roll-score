...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の端末別レイアウト補正までユーザー確認済みである。PC、スマートフォン、Androidタブレット、Windowsタブレットで概ね良好と確認された。第10段階へ進む前の最後の小補正として、スマートフォン横向きの縦表示だけ、画面下部の簡易鍵盤ガイド高さを圧縮し、音符が流れるCanvas領域を広げる実装も完了した。次はスマートフォン横向きの実機確認を行う。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: スマートフォン横向きの鍵盤ガイド高さ補正実装済み、ユーザー確認待ち
