...
このプロジェクトは現在、第12段階「拍線・小節線」まで完了している。MVP公開準備として自作ホームページのアプリディレクトリへ配置できる状態を整えたうえで、v0.2の最初の改善としてピアノ表示とスコア表示の両方に拍線・小節線を追加した。PC、スマートフォン、Androidタブレット、Windowsタブレットで主要レイアウトの確認は完了している。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- [docs/future-todos.md](docs/future-todos.md)
  - 第11.5段階以降で出た将来改善案と、第12段階で対応済みになった項目
