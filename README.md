...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の追加補正実装開始待ちである。再々補正により通常練習画面は改善されたが、表示調整モードではスライダー領域がCanvasを圧迫し、実鍵盤を見ながら調整しにくい。また、スマートフォンではテンポが速い課題の音符移動を目で追いにくいため、再生速度とは別に音価の幅、つまり時間方向の表示密度を調整できるようにする。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 追加補正実装開始待ち
