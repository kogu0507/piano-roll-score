...
このプロジェクトは現在、第9段階「レスポンシブ対応と操作性調整」まで実装済みで、PC・スマートフォン・可能であればタブレットでの手動確認待ちである。スマートフォン、タブレット、PCで既存機能を無理なく操作できるように画面全体を調整した。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
