...
第21段階では、右手版の「カエルの合唱」に続けて使える左手単独教材 `007: カエルの合唱 左手` を内蔵サンプルへ追加した。

- `007.json` は `clef: "bass"`、低音域C3〜A3、全音符 `hand: "left"` の導入サンプルである。
...
- `002: カエルの合唱` と `007: カエルの合唱 左手` は、同じ `seriesTitle: "カエルの合唱"` の教材としてカタログ上でまとまる。
- デモ教室カタログでは、カエルの合唱の右手版と左手版を同じシリーズ例として表示する。
- 既存IDの大規模リネーム、両手同時表示、大譜表、検索・フィルターUIは未実装である。
