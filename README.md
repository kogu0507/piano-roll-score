...
- [docs/future-todos.md](docs/future-todos.md)
  - 第11.5段階以降で出た将来改善案と、第12〜第22段階で対応済みになった項目
...
- [docs/manual-checklist-stage21.md](docs/manual-checklist-stage21.md)
  - 第21段階の左手単独教材と低音部スコア表示の確認手順
- [docs/manual-checklist-stage22.md](docs/manual-checklist-stage22.md)
  - 第22段階の教材バリエーションと運営教材カタログ整理の確認手順
...
```text
https://example.com/app/piano-roll-score/?id=000
```
...
- 第21段階 左手単独教材と低音部スコア表示の正式対応: 実装済み
- 第22段階 教材バリエーションと運営教材カタログ整理: 実装済み
- 次の作業: 教室カタログでの右手版・左手版の見え方確認、実際の教室コード、教材JSON、ローカル公開用 `/public_html/data/...` 配置での運用確認
...
## 第21段階メモ: 左手単独教材と低音部スコア表示

第21段階では低音部・左手教材の表示基盤を確認し、正式運用前の内蔵データ整理で「カエルの合唱」を`040`〜`049`へまとめた。

- `040.json` は右手版、`041.json`〜`049.json`は左手伴奏Lv.1〜9である。
- 左手伴奏は `clef: "bass"`、低音域C3〜G3、全音符 `hand: "left"` とする。
- 左手運指はC3・D3・E3・F3・G3の順に5・4・3・2・1とする。固定した右手メロディに対して、Lv.1〜9で異なる伴奏リズム・奏法を練習する。
- 左手伴奏はLv.1（`041`）のみホーム表示し、Lv.2〜9（`042`〜`049`）はカタログと直接URLから開く。
- 旧`007.json`の左手旋律移調版は本番用内蔵サンプルとして使用しない。
- スコア表示では `clef: "bass"` に基づき、ヘ音記号相当の低音部譜表位置へ音符を配置する。
- ピアノ表示では従来どおり実音高を基準にし、白鍵幅と譜面横位置の調整で左手低音域の実鍵盤合わせを行う。
- 大譜表、両手同時表示、音符ごとのclef切り替え、本格的な楽譜組版は未実装である。

## 第22段階メモ: 教材バリエーションと運営教材カタログ整理

第22段階では、右手版、左手版、将来の両手版、分割練習版をカタログ上で探しやすくするため、内蔵曲一覧と教室カタログへ任意の教材バリエーションメタデータを追加できるようにした。

- `seriesId`、`seriesTitle`、`part`、`variantLabel`、`sortOrder` は任意項目であり、既存の楽曲JSON形式やURLを変更しない。
- `040: カエルの合唱` と `041`〜`049`の左手伴奏は、同じ `seriesTitle: "カエルの合唱"` の教材としてカタログ上でまとまる。
- デモ教室カタログでは、カエルの合唱の右手版と左手版を同じシリーズ例として表示する。
- 正式運用前の整理として、基準版を10刻み、関連バリエーションを同じ10件の番号帯へ置くID体系へ移行した。
- 両手同時表示、大譜表、検索・フィルターUIは未実装である。

現在の内蔵教材は、`000: メリーさんの羊`、`010: ぶんぶんぶん`、`020: 聖者の行進`、`030: 喜びの歌`、`040`〜`049: カエルの合唱`、`050: きらきら星`の順に整理している。`901`、`902`はホーム非表示の開発確認用データとして維持する。

O-025 M04（[Issue #1](https://github.com/kogu0507/piano-roll-score/issues/1)）で旧`001`〜`007`のURL互換性は終了する。現行確認手順・検証結果・レビュー待ち事項は [O-025 M04実装記録](docs/o025-m04-implementation.md) を参照する。過去段階の資料に残る旧IDは履歴であり、現在の動作確認先ではない。
