...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の補正実装開始待ちである。直近実装では主要操作の集約は進んだが、意図していた「上部: 曲名・切替・メニュー、中央: Canvas最大表示、下部: 再生・表示調整バー」という画面構造にはなっていない。第10段階へ進む前に、スマートフォンやタブレットを鍵盤付近へ置いて使う前提の練習画面へ補正する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
