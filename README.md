...
このプロジェクトは現在、第9.5段階「練習画面レイアウト再設計」の再々補正実装が完了し、ユーザー確認待ちである。通常練習画面の上部操作はアイコン中心の最小操作列へ変更され、再生速度は選択式UI、曲の現在位置は低背シークバー、練習メニューは画面内固定オーバーレイになった。第10段階へ進む前に、PC、Android Chrome、可能であればタブレットで、Canvas表示領域、メニュー画面内表示、プルダウン更新の起きにくさを確認する。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
...
- 第9.5段階 練習画面レイアウト再設計: 再々補正実装済み、ユーザー確認待ち
