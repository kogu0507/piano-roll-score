...
このプロジェクトは現在、第9段階「レスポンシブ対応と操作性調整」まで実装済みで、第9.5段階「練習画面レイアウト再設計」の実装開始待ちである。次は、スマートフォンやタブレットを鍵盤付近へ置いて使う前提で、譜面Canvasと練習中によく使う操作を画面の主役にする。音源再生、演奏判定、クラウド同期は後続段階またはMVP対象外で扱う。
